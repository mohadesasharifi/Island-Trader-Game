...
public class CommandLines<reutrn> {
	...
	public void start(ArrayList<Islands> island, ArrayList<Ship> ship) {
		...
		if (input.equalsIgnoreCase("Y")) {
			System.out.println("\nDear " + name + "Welcome to the " + homeIsland(island) + "\n"+ ship.get(shipNo) + "\nYou have " + days + " days to travel." );
			startGame = input;
		}else {
			startGame = input;
		}
	...
	public void visitStore(ArrayList<Islands> island, Trader trader) {
		String input = "";
		String input2 = "";
		System.out.println("\nDear "+name + ", You can visit " +  island.get(currentIslandNo).getStore().getName() +
	...
	/**
	 * Trader choose an island to travel 
	 * destinationIsland calls chooseRoote 
	 * @param islands
	 * @param trader
	 */
...
//		&& !(check.notHomeIsland(currentIslandNo, islandNo))
	...
	/**
	 * chooseRoote is called in destinationIsland. 
	 * when Trader choose a destination to travel he chooses a route to travel.
	 * each route has a randomly selected chance of events occurring 
	 * @param source
	 * @param destination
	 * @param trader
	 * @param island
	...
	public void chooseRoute(int source, int destination, Trader trader, ArrayList<Islands> island) {
		...
		ArrayList routes = route.calculateAllPaths(source, destination);
		...
		if ( myRoute.size() <= days) {
			badWeather.attack(trader);
			shark.attack(trader);
			rescue.rescued(trader);
